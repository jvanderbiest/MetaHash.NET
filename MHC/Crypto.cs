...
namespace MHC
{
    ...
    public static class Crypto
    {
        ...
        private const string Sha256WithEcdsa = "SHA-256withECDSA";
        ...
        public static byte[] ParsePublicKey(string publicKey)
        {
            try
            {
                ...
                if (parser.ReadObject() is Asn1SequenceParser sequence)
                {
                    if (sequence.ReadObject() is DerObjectIdentifier oid && oid.Id == "1.2.840.10045.2.1")
                    {
                        oid = sequence.ReadObject() as DerObjectIdentifier;
                        if (oid != null && (oid.Id == "1.2.840.10045.3.1.7" || oid.Id == "1.3.132.0.10"))
                        {
                            if (sequence.ReadObject() is DerBitString bits) return bits.GetBytes();
                        }
                    }
                    return null;
                }
        ...
        /// </summary>
        public static string EncodePrivateKey(byte[] publicKey, byte[] privateKey)
        {
            // repack DER
            DerSequence seq = new DerSequence(
                new DerInteger(1),
        ...
        public static string EncodePrivateKey(byte[] privateKey)
        {
            ...
            return EncodePrivateKey(publicParams.Q.GetEncoded(), privateKey);
        }
        ...
        {
            var privKeyObj = Asn1Object.FromByteArray(Hex.Decode(privateKey));
            var privStruct = ECPrivateKeyStructure.GetInstance(privKeyObj);
            var algId = new AlgorithmIdentifier(X9ObjectIdentifiers.IdECPublicKey, privStruct.GetParameters());
            var privInfo = new PrivateKeyInfo(algId, privKeyObj);
            var par = PrivateKeyFactory.CreateKey(privInfo) as ECPrivateKeyParameters;
        ...
        {
            var par = PublicKeyFactory.CreateKey(Hex.Decode(publicKey)) as ECPublicKeyParameters;
            ISigner signer = SignerUtilities.GetSigner(Sha256WithEcdsa);
